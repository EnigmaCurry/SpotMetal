# XML generation
# Based on an example Created by jan@biochemfusion.com.

# A unique ID for the add-in.
addin_id = "com.enigmacurry.SpotMetal"
addin_version = "0.1"
addin_displayname = "SpotMetal - calc functions to get spot price for gold/silver"
addin_publisher_link = "http://www.enigmacurry.com"
addin_publisher_name = "Ryan McGuire"
...
add_manifest_entry(manifest_xml, 'uno-typelibrary;type=RDB', 'XSpotMetal.rdb')
add_manifest_entry(manifest_xml, 'configuration-data', 'CalcAddIn.xcu')
add_manifest_entry(manifest_xml, 'uno-component;type=Python', 'spot_metal.py')
manifest_xml.write('</manifest:manifest> \n')
...
def define_function(xml_file, function_name, description, parameters):
	...
	xml_file.write('    <prop oor:name="CompatibilityName"><value xml:lang="en">AutoAddIn.SpotMetal.' + function_name + '</value></prop>\n')
...
define_function(calcaddin_xml, \
	'spotMetal', 'Retrieves the current spot price of a metal in USD (cached)', \
	[('metal','The name of the metal'),('bidAsk','Specify "bid" or "ask" price')])
define_function(calcaddin_xml, \
	'spotMetalRefresh', 'Refresh the spot price cache immediately', \
	[])
